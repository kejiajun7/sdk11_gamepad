...
#include "main_init.h"
		...
		extern uint8_t GPIOTE_PORT_EVENT_FLAG;
    const uint8_t BUTTONS[] = INPUT_BUTTON;
//		static bool BUTTON_PRESS_FLAG[INPUTS_NUMBER] = {0};
static	bool home_press_flag = false;
	
		GPIOTE_PORT_EVENT_FLAG = 0x01;//Judge		
	
		for (i = 0; i < INPUTS_NUMBER; i++)
		{
				if(nrf_gpio_pin_read(BUTTONS[i]) == 0)
				{
						nrf_gpio_cfg_sense_input(BUTTONS[i],NRF_GPIO_PIN_PULLUP,GPIO_PIN_CNF_SENSE_High);
						if(BUTTONS[i] == BETOP_BUTTON)
						{
							GPIOTE_PORT_EVENT_FLAG |= 0x80;//Judge	
							home_press_flag = true;							
						}
						BUTTON_PRESS_FLAG[i] = true;
				}
				else
				{
						nrf_gpio_cfg_sense_input(BUTTONS[i],NRF_GPIO_PIN_PULLUP,NRF_GPIO_PIN_SENSE_LOW);
						if(BUTTONS[i] == BETOP_BUTTON)
						{
							if(home_press_flag == true)
							{
									home_press_flag = false;							
									GPIOTE_PORT_EVENT_FLAG |= 0x40;//Judge		
							}
						}
				}
...
//		if(GPIOTE_PORT_EVENT_FLAG == 1)GPIOTE_PORT_EVENT_FLAG = 0x11;//Judge
//		if(GPIOTE_PORT_EVENT_FLAG == 2)GPIOTE_PORT_EVENT_FLAG = 0x21;//Judge
