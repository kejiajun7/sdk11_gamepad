...
void gzp_init()
{
  ...
  gzp_host_chip_id_read(system_address, GZP_SYSTEM_ADDRESS_WIDTH);
	system_address[0] = 0X4D;
	system_address[1] = 0X54;
	system_address[2] = 0XBE;
	system_address[3] = 0X26;
...
static void gzp_process_address_req(uint8_t* gzp_req)
{
  ...
  if(prev_gzp_rx_info.rssi >= GZP_HOST_RX_POWER_THRESHOLD) 
  {
	...
	pairing_resp[0+GZP_CMD_HOST_ADDRESS_RESP_ADDRESS] = 0X4D;
	pairing_resp[1+GZP_CMD_HOST_ADDRESS_RESP_ADDRESS] = 0X54;
	pairing_resp[2+GZP_CMD_HOST_ADDRESS_RESP_ADDRESS] = 0XBE;
	pairing_resp[3+GZP_CMD_HOST_ADDRESS_RESP_ADDRESS] = 0X26;
