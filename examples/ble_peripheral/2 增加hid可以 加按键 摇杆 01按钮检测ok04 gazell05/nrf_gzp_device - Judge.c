...
//uint32_t send_success_count = 0;//Judge
//uint32_t receive_data_count = 0;//Judge
//uint32_t send_fail_count = 0;		//Judge

//uint8_t	pipe_num_assign;				//Judge
//uint8_t	pipe_use_state;						//Judge
//uint8_t	CMD_Value;							//Judge

extern  uint32_t send_success_count;	//Judge
extern  uint32_t receive_data_count;	//Judge
extern  uint32_t send_fail_count;		//Judge

extern uint8_t	pipe_num_assign;		//Judge
extern uint8_t	pipe_use_state;				//Judge
extern uint8_t	CMD_Value;					//Judge
...
//										pipe_num_assign = rx_payload[GZP_CMD_HOST_ADDRESS_RESP_PAYLOAD_LENGTH + 1];		//Judge
//										CMD_Value= rx_payload[GZP_CMD_HOST_ADDRESS_RESP_PAYLOAD_LENGTH + 2];					//Judge
										pipe_num_assign = rx_payload[5] & 0x0f;		//Judge
										CMD_Value= rx_payload[6];					//Judge
										pipe_use_state = rx_payload[5] >> 4;		//Judge
...
//    else if((retval & 0xf0) != 0xf0)  //Judge
//    {
//        retval >>= 4;
//    }
//    else
...
//          }
					pipe_use_state = db_byte & 0x0f; 
          db_byte = (db_byte >> 4) & 0x0f; //Judge
...
static bool gzp_index_db_full()
{
    ...
    return ((*(uint8_t*)(GZP_INDEX_DB_ADR + (GZP_INDEX_DB_SIZE - 1)) != 0xff));//第14条 有数据  代表满了
#else
    return true;
#endif
}
...
static bool gzp_params_store(bool store_all)
{
    ...
    if(write_index_db)
    {
        if(!gzp_index_db_full() && (new_db_index != (gzp_index_db_read() >> 4)) && (new_db_index != GZP_PARAMS_DB_MAX_ENTRIES))
        {
